...
call, not the Anthropic SDK - the gateway at ANTHROPIC_BASE_URL (currently
Orbio, api.orbio.so) exposes an OpenAI-compatible `/chat/completions`
endpoint, and this script's only job is to confirm that endpoint is
reachable with these credentials before anything else in the backend is
built on top of it.
...
# if you point ANTHROPIC_BASE_URL at a different gateway/model.
...
def main() -> int:
    ...
    # include /v1 in ANTHROPIC_BASE_URL itself. The OpenAI SDK appends
    ...
    try:
        ...
        if e.status_code in (400, 401):
            print(
                ...
                f"{MODEL!r} is valid for this gateway and that ANTHROPIC_BASE_URL "
                "/ ANTHROPIC_AUTH_TOKEN in .env match the provider you intend to "
